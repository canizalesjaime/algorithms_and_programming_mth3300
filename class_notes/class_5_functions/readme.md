...
### `len()` in Python returns the **number of items** in a container.
...
## Key points
- Returns an integer
- The object must have a defined length
- Otherwise you get a TypeError

In simple terms: len() counts how many items are inside something.
...
| `len(obj)` | length | `len('apple')` | `5` |


## Random Numbers
Python uses pseudorandom numbers — deterministic sequences that appear random.  
They’re generated by the built-in `random` module.
...
Same seed → same results (useful for testing and experiments).
...
### 6 
Write a Python program that generates a random integer between 1 and 100 (inclusive).

The program should then:
1. Print the number.
2. Print whether the number is even or odd.
3. Print whether the number is greater than 50 or not.

Example output (your number will be different each run):
